...
export const generateRandomModel = async (apiKey: string, country: string, isSensual: boolean, overallStyle: 'modern' | 'authentic', modelType: 'professional' | 'natural'): Promise<Partial<ModelData>> => {
  ...
  const ethnicFeaturesGuide = ETHNICITY_FEATURES_MAP[country] || 'A diverse range of human features.';
  const poseOptions = isSensual ? SENSUAL_POSES : NON_SENSUAL_POSES;
  const posePrompt = `Choose an appropriate pose from this list: [${poseOptions.map(p => `"${p}"`).join(', ')}]. The chosen pose should match the persona (${modelType}) and the overall mood.`;
  ...
  **Your Task:**
  ...
  - "description": Based on the guide above, create a brief, evocative description of a **unique individual's** facial features. Be **hyper-specific**. DO NOT repeat the guide; create a concrete example of a person who fits within it.
    - **BAD (Generic):** 'A woman with dark hair.'
    - **GOOD (Specific & Authentic, for Japanese):** 'A woman with a heart-shaped face, soft monolids that curve upwards slightly, and a small, delicate nose.'
  - "gender": Choose one from: ${GENDER_OPTIONS.join(', ')}.
  - "age": A number between 18 and 50.
    ...
    config: {
      ...
      responseSchema: {
        ...
        properties: {
          ...
          pose: { type: Type.STRING, enum: poseOptions },
        },
